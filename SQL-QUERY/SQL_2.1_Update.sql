...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_ViewCourse` (
		IN  p_Authenticate_Teacher_ID INT, 
	    IN  p_ID            INT)
BEGIN
	SELECT * FROM mydb.Course 
    WHERE Course.ID = p_ID AND (Course.Course_Status <> 'Discarded'
    OR (Course.Course_Status = 'Discarded' AND Course.Teacher_ID = p_Authenticate_Teacher_ID  ));
END $$
